...
function parseData(data, health) {
    return data.split('\n')
        .map(line => line
            .split('')
            .map(symbol => {
...
function runSim(health) {
    const map = parseData(data, health);
    const initialElves = countType(map, UNITS.ELF);
    ...
    while (true) {
        ...
        if (countType(map, UNITS.ELF) < initialElves) {
            return false;
        }
...
for (let i = 5; ; i++) {
    console.log(`Trying ${i} attack damage.`);
    if (runSim(i)) {
        break;
    }
}